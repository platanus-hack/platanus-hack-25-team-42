...
export const Route = createFileRoute("/_authflow/login")({
  ...
  loader: async ({ location }) => {
    ...
    try {
      const oauthApp = await getOAuthApplicationByClientId({
        data: { clientId },
      });
      return { oauthApp };
    } catch (error) {
...
function RouteComponent() {
  ...
  const { oauthApp } = Route.useLoaderData();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 w-full">
      <div className="max-w-md w-full space-y-8">
        ...
        {oauthApp && (
          <div className="flex flex-col items-center space-y-3 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            <Avatar.Root className="w-16 h-16 rounded-lg border border-gray-200 shadow-md overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
              ...
              <Avatar.Fallback className="w-full h-full flex items-center justify-center text-2xl font-bold text-gray-500">
                {(oauthApp.name || "A").charAt(0).toUpperCase()}
              </Avatar.Fallback>
            </Avatar.Root>
            <div className="text-center">
              <p className="text-sm text-gray-600">Iniciar sesión en</p>
